...
IMPLEMENT_DYNAMIC(CScreenshotToolDlg, CDialogEx)
	...
	: CDialogEx(CScreenshotToolDlg::IDD, pParent)
{
	...
	//{{AFX_DATA_INIT(CScreenshotToolDlg)
...
BEGIN_MESSAGE_MAP(CScreenshotToolDlg, CDialogEx)
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_WM_MOUSEMOVE()
	ON_WM_LBUTTONDOWN()
...
void CScreenshotToolDlg::OnCancel() 
{
...
HBRUSH CScreenshotToolDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor) 
{
...
//*********************增加的函数**********************************************************
//考贝屏幕,这段代码是拿来主义 呵呵
HBITMAP CScreenshotToolDlg::CopyScreenToBitmap(LPRECT lpRect,BOOL bSave)
//lpRect 代表选定区域
{
	HDC       hScrDC, hMemDC;      
	// 屏幕和内存设备描述表
...
//显示操作提示信息
void CScreenshotToolDlg::DrawTip()
{
...
//显示截取矩形信息
void CScreenshotToolDlg::DrawMessage(CRect &inRect,CDC * pDC)
{
	//截取矩形大小信息离鼠标间隔
	const int space=3;
	...
	size=pDC->GetTextExtent(L"顶点位置  ",wcslen(L"顶点位置  "));
	lineLength=size.cx;
